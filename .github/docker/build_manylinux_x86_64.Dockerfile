...
# install python-shared
RUN dnf install -y wget libffi-devel openssl-devel bzip2-devel ncurses-devel
RUN dnf install -y gdbm-devel xz-devel tcl-devel tk-devel libuuid-devel libnsl2-devel

ENV PYTHON_VERSION 3.10.19
ENV PYTHON_INSTALL_BASE=/opt/python-shared
ENV PYTHON_INSTALL_SRC_DIR=${PYTHON_INSTALL_BASE}/Python-${PYTHON_VERSION}
ENV PYTHON_INSTALL_SRC_ARCHIVE=${PYTHON_INSTALL_SRC_DIR}.tgz
ENV PYTHON_INSTALL_DIR=${PYTHON_INSTALL_BASE}/cp310-cp310

# first delete previous version of python in install dir
# and then download, extract, configure, compile and install Python from source
# and finally create also python to python3 symlinks and delete the source code
RUN rm -rf ${PYTHON_INSTALL_DIR} \
    && mkdir -p ${PYTHON_INSTALL_BASE} \
    && cd ${PYTHON_INSTALL_BASE} \
    && wget -c https://www.python.org/ftp/python/${PYTHON_VERSION}/Python-${PYTHON_VERSION}.tgz \
    && tar -xzf Python-${PYTHON_VERSION}.tgz -C ${PYTHON_INSTALL_BASE} \
    && rm -f ${PYTHON_INSTALL_SRC_ARCHIVE} \
    && cd ${PYTHON_INSTALL_SRC_DIR} \
    && CFLAGS="-fPIC" CXXFLAGS="-fPIC" ./configure --prefix=${PYTHON_INSTALL_DIR} --enable-optimizations \
    && make -j$(nproc) \
    && make install \
    && ln -s pip3 ${PYTHON_INSTALL_DIR}/bin/pip \
    && ln -s pydoc3 ${PYTHON_INSTALL_DIR}/bin/pydoc \
    && ln -s python3 ${PYTHON_INSTALL_DIR}/bin/python \
    && ln -s python3-config ${PYTHON_INSTALL_DIR}/bin/python-config \
    && rm -rf ${PYTHON_INSTALL_SRC_DIR}

ENV PYTHON_VERSION 3.11.14
ENV PYTHON_INSTALL_BASE=/opt/python-shared
ENV PYTHON_INSTALL_SRC_DIR=${PYTHON_INSTALL_BASE}/Python-${PYTHON_VERSION}
ENV PYTHON_INSTALL_SRC_ARCHIVE=${PYTHON_INSTALL_SRC_DIR}.tgz
ENV PYTHON_INSTALL_DIR=${PYTHON_INSTALL_BASE}/cp311-cp311
...
ENV PYTHON_INSTALL_SRC_DIR=${PYTHON_INSTALL_BASE}/Python-${PYTHON_VERSION}
ENV PYTHON_INSTALL_SRC_ARCHIVE=${PYTHON_INSTALL_SRC_DIR}.tgz
ENV PYTHON_INSTALL_DIR=${PYTHON_INSTALL_BASE}/cp312-cp312
...
ENV PYTHON_INSTALL_SRC_DIR=${PYTHON_INSTALL_BASE}/Python-${PYTHON_VERSION}
ENV PYTHON_INSTALL_SRC_ARCHIVE=${PYTHON_INSTALL_SRC_DIR}.tgz
ENV PYTHON_INSTALL_DIR=${PYTHON_INSTALL_BASE}/cp313-cp313
...
ENV PYTHON_INSTALL_SRC_DIR=${PYTHON_INSTALL_BASE}/Python-${PYTHON_VERSION}
ENV PYTHON_INSTALL_SRC_ARCHIVE=${PYTHON_INSTALL_SRC_DIR}.tgz
ENV PYTHON_INSTALL_DIR=${PYTHON_INSTALL_BASE}/cp314-cp314
